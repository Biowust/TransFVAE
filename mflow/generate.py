...
def visualize_interpolation(filepath, model, mol_smiles=None, mols_per_row=5,
    ...
    if mol_smiles is not None:  # Not used in this file
        ...
        with torch.no_grad():
            ...
            fp0 = AllChem.GetMorganFingerprint(mol0, radius=2)
            ...
            z0, _ = model.forward1(adj, x, adj_normalized)  # [h:(1,45), adj:(1,324)], [sum_log_det_jacs_x: (1,), sum_log_det_jacs_adj: (1,)]
...
#             mol_index = [4546, 77955]
            ...
            z0, _ = model.forward1(adj0, x0, adj_normalized0)  # [h:(1,45), adj:(1,324)], [sum_log_det_jacs_x: (1,), sum_log_det_jacs_adj: (1,)]
            # z0 = np.hstack((z0[0][0].data, z0[0][1].data)).squeeze(0)
            z0[0] = z0[0].reshape(z0[0].shape[0], -1)
            z0[1] = z0[1].reshape(z0[1].shape[0], -1)
            # z0 = torch.cat((z0[0][0], z0[0][1]), dim=1).squeeze(dim=0) # h:(1,45), adj:(1,324) -> (1, 369) -> (369,)
            ...
            z1, _ = model.forward1(adj1, x1,adj_normalized1)  # [h:(1,45), adj:(1,324)], [sum_log_det_jacs_x: (1,), sum_log_det_jacs_adj: (1,)]
            # z0 = np.hstack((z0[0][0].data, z0[0][1].data)).squeeze(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_name', type=str, default='zinc250k', choices=['qm9', 'zinc250k', 'ours'], help='dataset name')
    # parser.add_argument('--molecule_file', type=str, default='qm9_relgcn_kekulized_ggnp.npz',
    #                     help='path to molecule dataset')
    parser.add_argument("--snapshot-path", default="-snapshot", type=str, required=True)
...
#         print('model.ln_var[0]: {:.2f}, model.ln_var[1]: {:.2f}'.format(model.ln_var[0].item(), model.ln_var[1].item()))
    ...
    if args.reconstruct:
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            graph_data = batch[2]
            z, _, _, = model(graph_data, adj, x, adj_normalized)
