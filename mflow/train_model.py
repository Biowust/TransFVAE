...
def get_parser():
    ...
    parser.add_argument('-o', '--save_dir', type=str, default='results/zinc250k_512t2cnn_256gnn_512-64lin_10flow_19fold_convlu2_38af-1-1mask',
    ...
    parser.add_argument('--save_epochs', type=int, default=1, help='in how many epochs, a snapshot of the model'
                                                                   ' needs to be saved?')
    # data loader
    parser.add_argument('-b', '--batch_size', type=int, default=512, help='Batch size during training per GPU')
    ...
    parser.add_argument('--a_hidden_gnn', type=str, default="256,",
                        help='Hidden dimension list for graph convolution for atoms matrix, delimited list input ')
    parser.add_argument('--a_hidden_lin', type=str, default="512,64",
    ...
    for epoch in range(args.max_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            for key, value in batch[2].items():
                batch[2][key] = value.to(device)
            graph_data = batch[2]
            # Forward, backward and optimize
            z, sum_log_det_jacs, vae_para = model(graph_data, adj, x, adj_normalized)
