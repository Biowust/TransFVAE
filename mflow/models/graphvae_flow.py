...
class MoFlow(nn.Module):
    def __init__(self, hyper_params: Hyperparameters):
        ...
        self.graphormer_encoder = Graphormer(n_layers=12,
                                             head_size=8,
                                             hidden_dim=80,
                                             dropout_rate=0.1,
    ...
    def forward(self, graph_data, adj, x, adj_normalized, device=-1):
        """
        :param adj:  (256,4,9,9)
        :param x: (256,9,5)
        :return:
        """
        z, z_mu, z_logvar = self.graphormer_encoder(graph_data)  # (batch_size, 4*9*9+9*5)
    ...
    def forward1(self, adj, x, adj_normalized, device=-1):
        """
        :param adj:  (256,4,9,9)
        :param x: (256,9,5)
        :return:
