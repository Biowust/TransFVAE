...
from mflow.utils.environment import calculateScore
#from sklearn.linear_model import LinearRegression
import time
import functools
print = functools.partial(print, flush=True)

def penalized_logp(moll):
    """
    ...
    # cycle score
    res = Chem.rdmolops.GetAdjacencyMatrix(mol)
    a = nx.Graph(res)
    cycle_list = nx.cycle_basis(a)
    if len(cycle_list) == 0:
        cycle_length = 0
    else:
...
class MoFlowProp(nn.Module):
    ...
    def encode(self, adj, x):
        with torch.no_grad():
            ...
            z, sum_log_det_jacs = self.model.forward1(adj, x, adj_normalized)
...
def optimize_mol(model:MoFlow, property_model:MoFlowProp, smiles, device, sim_cutoff, lr=2.0, num_iter=20,
    ...
    if property_name == 'qed':
        ...
        propf = penalized_logp  # unbounded, normalized later???
    else:
        raise ValueError("Wrong property_name{}".format(property_name))
#     model = model.to(device)
#     property_model = property_model.to(device)
    ...
    with torch.no_grad():
        ...
        if debug:
            ...
            z, sum_log_det_jacs = model.forward1(bond, atoms, adj_normalized)
